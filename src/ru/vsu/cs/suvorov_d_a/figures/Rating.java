...
import ru.vsu.cs.suvorov_d_a.game.AlphaBetaChess;
    ...
    public static int rateAttack(){
        ...
        for (int i=0;i<64;i++) {
            switch (AlphaBetaChess.chessBoard[i / 8][i % 8]) {
                case "P" -> {
                    AlphaBetaChess.kingPositionA = i;
    ...
    public static int ratePositional(int material) {
        ...
        for (int i=0;i<64;i++) {
            switch (AlphaBetaChess.chessBoard[i / 8][i % 8]) {
                ...
                case "A" -> {
                    if (material >= 1750) {
                        rating += kingMidBoard[i / 8][i % 8];
                        rating += AlphaBetaChess.possibleA(AlphaBetaChess.kingPositionA).length() * 10;
                    } else {
                        rating += kingEndBoard[i / 8][i % 8];
                        rating += AlphaBetaChess.possibleA(AlphaBetaChess.kingPositionA).length() * 30;
                    }
                }
            }
        }
