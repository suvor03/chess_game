package ru.vsu.cs.suvorov_d_a.game;

import ru.vsu.cs.suvorov_d_a.figures.Rating;
...
public class AlphaBetaChess {
    ...
    public static int kingPositionA;
    static int kingPositiona; //white and black king's position in the array
    ...
    public static String alphaBeta(int depth, int beta, int alpha, String move, int player) {
        ...
        if(depth==0 || moveList.length() == 0)	//limiting cases if searching depth becomes 0 or no possible moves are left (e.g. checkmate)
        ...
        for(int i = 0; i < moveList.length(); i += 5){
            ...
            ratingValue = Integer.parseInt(returnedStr.substring(5));	//get the value of the rating from the depth
            flipBoard();
		...
		*/
        for (int i=0; i<64; i++) {
            switch (chessBoard[i / 8][i % 8]) {
                case "P" -> moveList.append(possibleP(i));
                case "R" -> moveList.append(possibleR(i));
                case "K" -> moveList.append(possibleK(i));
                case "B" -> moveList.append(possibleB(i));
                case "Q" -> moveList.append(possibleQ(i));
                case "A" -> moveList.append(possibleA(i));
            }
        }
    ...
    public static String possibleP(int i){
        ...
        for(int j=-1;j<=1;j+=2){
            try {	//finding possible captures diagonally
                if(Character.isLowerCase(chessBoard[r-1][c+j].charAt(0)) && i>=16){
                    ...
                    if(kingSafe()){
                        moveList.append(r).append(c).append(r - 1).append(c + j).append(oldPiece);
                    }
                    chessBoard[r][c]="P";
                ...
                if(Character.isLowerCase(chessBoard[r-1][c+j].charAt(0)) && i<16){
                    ...
                    for(int k=0;k<4;k++){
                        ...
                        if(kingSafe()){
                            ...
                            moveList.append(c).append(c + j).append(oldPiece).append(temp[k]).append("P");
                        }
                        chessBoard[r][c]="P";
                        chessBoard[r-1][c+j]=oldPiece;
                    }
        ...
        try {//move one up
            if (" ".equals(chessBoard[r-1][c]) && i>=16) {
                ...
                chessBoard[r-1][c]="P";
                if (kingSafe()) {
                    moveList.append(r).append(c).append(r - 1).append(c).append(oldPiece);
                }
                chessBoard[r][c]="P";
        ...
        try {//promotion && no capture
            if (" ".equals(chessBoard[r-1][c]) && i<16) {
                ...
                for (int k=0; k<4; k++) {
                    ...
                    if (kingSafe()) {
                        //old Column position, new Column position, captured piece, new piece, P(promotion)
                        moveList.append(c).append(c).append(oldPiece).append(temp[k]).append("P");
                    }
                    chessBoard[r][c]="P";
                    chessBoard[r-1][c]=oldPiece;
                }
        ...
        try {//move two up
            if (" ".equals(chessBoard[r-1][c]) && " ".equals(chessBoard[r-2][c]) && i>=48) {
                ...
                chessBoard[r-2][c]="P";
                if (kingSafe()) {
                    moveList.append(r).append(c).append(r - 2).append(c).append(oldPiece);
                }
                chessBoard[r][c]="P";
        ...
        for(int j=-1;j<=1;j+=2){
            try {
                while(" ".equals(chessBoard[r][c+temp*j])){	//horizontal direction
                    ...
                    if(kingSafe()){
                        moveList.append(r).append(c).append(r).append(c + temp * j).append(oldPiece);
                    }
                    chessBoard[r][c]="R";
            ...
            try {
                while(" ".equals(chessBoard[r+temp*j][c])){	//vertical direction
                    ...
                    if(kingSafe()){
                        moveList.append(r).append(c).append(r + temp * j).append(c).append(oldPiece);
                    }
                    chessBoard[r][c]="R";
            ...
            for(int k=-1;k<=1;k+=2){
                try {
                    if (Character.isLowerCase(chessBoard[r + j][c + k * 2].charAt(0)) || " ".equals(chessBoard[r + j][c + k * 2])) {
                        ...
                        if (kingSafe()) {
                            moveList.append(r).append(c).append(r + j).append(c + k * 2).append(oldPiece);
                        }
                ...
                try {
                    if(Character.isLowerCase(chessBoard[r+j*2][c+k].charAt(0)) || " ".equals(chessBoard[r+j*2][c+k])){
                        ...
                        if(kingSafe()){
                            moveList.append(r).append(c).append(r + j * 2).append(c + k).append(oldPiece);
                        }
                ...
                if(j!=0 || k!=0){
                    try {
                        while(" ".equals(chessBoard[r+temp*j][c+temp*k])){
                            ...
                            if(kingSafe()){
                                moveList.append(r).append(c).append(r + temp * j).append(c + temp * k).append(oldPiece);
                            }
            ...
            if(j!=4){		//avoid checking for the place where KING is already placed
                try {
                    if(Character.isLowerCase(chessBoard[r-1+j/3][c-1+j%3].charAt(0)) || " ".equals(chessBoard[r-1+j/3][c-1+j%3])){
                        ...
                        if(kingSafe()){
                            moveList.append(r).append(c).append(r - 1 + j / 3).append(c - 1 + j % 3).append(oldPiece);
                        }
                        chessBoard[r][c]="A";
